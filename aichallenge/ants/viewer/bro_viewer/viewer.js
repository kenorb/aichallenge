...
Ants.parseData = function (data) {
    ...
    for (var l = 0, llen = lines.length; l < llen; ++l) {
        ...
        if (tokens[0] === 'turn') {
            ...
            /*
            globalData[turn] = {"#set":"1"};
            if (isNaN(globalData[turn]["total_ants"])) {
               
               globalData[turn]["total_ants"] = 1;
               alert("turn " + turn + " was nan, now is: " + globalData[turn]["total_ants"]);
            }
            globalData[turn]["total_ants"] = 1;
*/
...
var init = function () {
    ...
    var show_turn = function () {
        $('#turn').html('Turn ' + Ants.turn + ' / ' + Ants.max_turns);
        if (Ants.turn === 0) {
            $('#turn').append(' (Start)');
    ...
    var onTurnChange = function() {
        ...
        $('#globalData').html("");
        $.each(globalData[Ants.turn], function(key) {
          $('#globalData').append("<p>" + key + " = \"" + this + "\"</p>");
        });
    }
